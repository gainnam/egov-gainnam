...
JQuery(function($){
	...
	//반응형 코딩 추가(사용자 창크기를 마음대로 조정 시 height값을 자동 조정되게)
...
<div id="container">
			...
			<div class="rollbtnArea">
				<ul class="rollingbtn">
					<li class="seq butt0"><a href="#butt"><img
							src="<c:url value='/' />resources/home/img/btn_rollbutt_on.png"
							alt="1번" /></a></li>
					<li class="seq butt1"><a href="#butt"><img
							src="<c:url value='/' />resources/home/img/btn_rollbutt_off.png"
							alt="2번" /></a></li>
					<li class="seq butt2"><a href="#butt"><img
							src="<c:url value='/' />resources/home/img/btn_rollbutt_off.png"
							alt="3번" /></a></li>
					<li class="rollstop"><a href="#" class="stop"><img
							src="<c:url value='/' />resources/home/img/btn_roll_stop.png"
							alt="멈춤" /></a></li>
					<li class="rollplay"><a href="#" class="play"><img
							src="<c:url value='/' />resources/home/img/btn_roll_play.png"
							alt="재생" /></a></li>
				</ul>
			</div>
			<!-- //슬라이드버튼영역 -->
	...
	<div class="about_area">
		<h2>
			<a
				href="<c:url value='/tiles/board/list_board.do?bbsId=BBSMSTR_BBBBBBBBBBBB' />">
			...
			<ul class="place_list box_inner clear">
				<c:forEach items="${galleryList}" var="galleryVO">
					<li class="view_detail" style="cursor: pointer">
						<form name="view_form"
							action="<c:url value='/tiles/board/view_board.do' />"
							method="post">
							<div class="fix_height">
							<c:if test="${empty galleryVO.atchFileId}">
								<img class="img_topplace"
									src="<c:url value='/' />resources/home/img/no_image.png"
									alt="OOOO OOOOO" />
							</c:if>
							<c:if test="${not empty galleryVO.atchFileId}">
								<img class="img_topplace" alt="첨부파일 이미지"
									src="<c:url value='/tiles/board/previewImage.do' />?atchFileId=${galleryVO.atchFileId}"
									 />
							</c:if>
							</div>
							...
							<span class="view">VIEW</span> <input type="hidden" name="bbsId"
								value="<c:out value='${galleryVO.bbsId}'/>" /> <input
								type="hidden" name="nttId"
								value="<c:out value="${galleryVO.nttId}"/>" /> <input
								...
								type="hidden" name="authFlag" value="<c:out value='Y'/>" /> <input
								name="pageIndex" type="hidden" value="<c:out value='1'/>" />
						</form>
					</li>
				</c:forEach>
			</ul>
					...
					<c:forEach items="${noticeList}" var="noticeVO">
						<li class="view_detail" style="cursor:pointer;">
							...
							<input type="hidden" name="bbsId"
								value="<c:out value='${noticeVO.bbsId}'/>" /> <input
								type="hidden" name="nttId"
								value="<c:out value="${noticeVO.nttId}"/>" /> <input
								type="hidden" name="bbsTyCode"
								value="<c:out value='${noticeVO.bbsTyCode}'/>" /> <input
								type="hidden" name="bbsAttrbCode"
								value="<c:out value='${noticeVO.bbsAttrbCode}'/>" /> <input
								type="hidden" name="authFlag" value="<c:out value='Y'/>" /> <input
								name="pageIndex" type="hidden" value="<c:out value='1'/>" />
							</form>
						</li>
